...
first case. The reported p50/p95/mean are the per-step stage sum
(prefix_embed + prefix_lm + suffix_loop), which equals `infer_ms` when the
engines are co-resident — the quantity the AGX gate records — and stays the
pure compute when a board has to sequence the engines. `run_*` (the full
run_once, reloads included) and `initial_load_ms` are kept beside it: on the
8 GB NX one sequential run costs ~8.2 s against ~0.7 s of compute.
...
from pi_cpp.verify.gates import gates_for
from pi_cpp.verify.metrics import evaluate_latency, evaluate_parity, raw_max_abs, triplet

SURFACE_SHAPE = (50, 16)
STAGE_KEYS = ("load_ms", "prefix_embed_ms", "prefix_lm_ms", "suffix_loop_ms", "infer_ms")
...
def run_parity(command: VerifyConfig, gates: dict, report: dict) -> int:
    ...
    runner = picpp.Pi05OfflineRunner(command.package / "engines", residency=command.resident)
    runner.load()
    surfaces = {}
    ...
    observed = {
        ...
        "worst_case_relative_l2": comparison["worst_case_relative_l2"],
...
def run_latency(command: VerifyConfig, gates: dict, report: dict) -> int:
    ...
    warmup = command.warmup if command.warmup is not None else int(protocol.get("warmup", 10))
    runs = command.runs if command.runs is not None else int(protocol.get("runs", 100))
    ...
    for _ in range(runs):
        start = perf_counter()
        result = runner.run_once(inputs)
        wall_ms.append((perf_counter() - start) * 1000.0)
        for key in STAGE_KEYS:
            value = getattr(result, key, None)
            if value is not None:
                stage_ms.setdefault(key, []).append(float(value))
        run_ms.append(float(result.infer_ms))
    step_columns = zip(*(stage_ms[key] for key in ("prefix_embed_ms", "prefix_lm_ms", "suffix_loop_ms")))
    step_values = np.asarray([sum(columns) for columns in step_columns], dtype=np.float64)
    latency = {
        **_percentiles(step_values),
        "measured": "per-step prefix_embed + prefix_lm + suffix_loop; equals infer_ms when co-resident",
        "residency": command.resident,
        "warmup_runs": warmup,
        "measure_runs": runs,
        "run_p50_ms": float(np.percentile(np.asarray(run_ms, dtype=np.float64), 50)),
        "run_mean_ms": float(np.mean(run_ms)),
        "wall_p50_ms": float(np.percentile(np.asarray(wall_ms, dtype=np.float64), 50)),
        "wall_mean_ms": float(np.mean(wall_ms)),
        "stage_mean_ms": {key: float(np.mean(values)) for key, values in stage_ms.items()},
        "initial_load_ms": float(stage_ms["load_ms"][0]),
        "samples_ms": [float(value) for value in step_values],
        "run_samples_ms": run_ms,
    }
...
def run_verify(command: VerifyConfig) -> int:
    ...
    report: dict = {"mode": command.mode, "family": family, "residency": command.resident}
...
def main(argv: list[str] | None = None) -> int:
    return run_verify(tyro.cli(VerifyConfig, args=argv, prog="picpp verify"))
